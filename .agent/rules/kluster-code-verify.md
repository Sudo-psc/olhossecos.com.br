...
- If no files were changed, do **not** produce code; instead, state: _“No code changes; verification not required.”_
...
- If the agent makes **multiple edits** within one turn, it must run **one** `kluster_open_snapshot_session` (first) and **one** `kluster_code_review_auto` **after the last edit** _and_ after completing all todos.
